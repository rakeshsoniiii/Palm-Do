...
class FingerCounter:
    ...
    def is_thumb_open(self, landmarks):
        ...
        # Distance from thumb CMC to palm center (reference)
        cmc_to_palm = math.sqrt((thumb_cmc.x - palm_center_x)**2 + 
                                (thumb_cmc.y - palm_center_y)**2)
        
        # Thumb is open if tip is much farther from palm than base
        ...
        # When folded, tip gets closer to MCP
        extension_check = tip_to_mcp > ip_to_mcp * 1.3
        
        # Method 3: Check horizontal distance (for right hand)
        # Thumb tip should be significantly to the left of thumb MCP when extended
        ...
        # When extended, tip is farther from index base than IP joint is
        separation_check = tip_to_index > ip_to_index * 0.9
        
        # Thumb is considered open only if multiple checks pass
    ...
    def is_finger_open(self, landmarks, finger_idx):
        ...
        # Check 1: Tip should be above PIP (lower y value)
        if tip_pos.y > pip_pos.y:
            return False
        
        # Check 2: Tip should be further from wrist than PIP
        ...
        # Check 3: The finger should be relatively straight
        ...
        # If finger is straight, tip_to_mcp should be close to sum of segments
    ...
    def announce(self, count):
        ...
        # Only announce if count changed and enough time has passed
            ...
            if results.multi_hand_landmarks and results.multi_handedness:
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                    ...
                    if hand_label == "Right":
                        ...
                        # Stability check - only announce if count is stable
